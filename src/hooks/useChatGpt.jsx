...
const useChatGpt = () => {
  ...
  const askChatGpt = async (title, subject, content, byte) => {
    ...
    let messages = [
      { role: "system", content: `당신은 학생의 ${subject} 과목 세부 능력 특기사항을 쓰는 교사임. ${title} 활동에서 학생이 할 수 있는 활동을 적고 그 활동이 학생에게 미친 영향을 3자의 입장에서 객관적으로 적어야 함. 모든 문장을 '~음', '~했음', '~임', '~함' 과 같이 개조식으로 작성해야 함. ` },
      { role: "user", content: "여행지 소개하기 활동을 수행한 학생이 받을 생기부 세특 예시를 작성해 주세요." },
      { role: "assistant", content: "여행지 소개하기 활동에서 가보고 싶은 해외 도시로 LA를 선정하여, 그 지역 명소인 테마파크의 실제 리뷰를 해외 사이트에서 찾아, 이를 바탕으로 여행지를 소개하는 글을 작성하였고 구체적인 여행 계획을 세움. 이 활동을 통해 실제 인터넷에서 쓰이는 표현에 대해 알 수 있었다고 작성하였고 여행 중 마주칠 수 있는 상황에 대한 문제 해결력을 증명함. " },
