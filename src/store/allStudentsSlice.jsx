...
let allStudents = createSlice({
  ...
  reducers: {
    ...
    setEditHomeroomStudent(state, action) { //state는 전역변수 현재 상태
      const { id, behaviorOpinion, selectedSpec } = action.payload
      let selected = state.filter(student => student.id === id)       //선택 된 학생 추출하여 새로운 배열 생성
      let unSelected = state.filter(student => student.id !== id)     //선택되지 않은 학생만 추출하여 새로운 배열 생성
      let edited = { ...selected[0], behaviorOpinion, selectedSpec }  //새롭게 학생 정보 수정
      unSelected.push(edited)
      return unSelected.sort((a, b) => a.studentNumber.localeCompare(b.studentNumber)) //학번으로 오름차순 정렬
    }
  }
})

export const { setAllStudents, setModifiedStudent, setEditHomeroomStudent } = allStudents.actions
